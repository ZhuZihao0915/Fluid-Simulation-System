...
namespace Glb {
    ...
    CubicSpline2d::CubicSpline2d(float h) {
        mH = h;
        mH2 = h * h;
    ...
    float CubicSpline2d::Value(float distance) {
        float res = 0;
    ...
    glm::vec2 CubicSpline2d::Grad(glm::vec2 radius) {
    ...
    float CubicSpline2d::CalculateValue(float distance) {
    ...
    glm::vec2 CubicSpline2d::CalculateGrad(glm::vec2 radius) {
        glm::vec2 res(0.0f, 0.0f);
    ...
    CubicSpline3d::CubicSpline3d(float h) {
        mH = h;
        mH2 = h * h;
        mH3 = mH2 * h;
    ...
    float_t* CubicSpline3d::GetData() {
        return (float_t*)mValueAndGradFactorBuffer.data();
    }
    

    uint32_t CubicSpline3d::GetBufferSize() {
        return mBufferSize;
    }

    glm::vec2 CubicSpline3d::GetValueAndGrad(float f) {
    ...
    float CubicSpline3d::CalculateValue(float distance) {
        float r = std::abs(distance);
        float q = r / mH;
        float q2 = q * q;
        float q3 = q * q2;
    ...
    float CubicSpline3d::CalculateGradFactor(float distance) {
        float res = 0.0f;
        if (distance < 1e-5) {
            return res;
        }
